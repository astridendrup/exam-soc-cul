...
library(tidyverse); library(stringr) ; library(data.table)
...
test <- subset(eventwords, X.run.number.== 11)
#define [] as NA
test[test == "[]"] <- NA
# exclude NAs
counteventword <- subset(test, eventword != "NA")
counteventword <- subset(counteventword, eventword != "0")

wordcounts <- subset(test, nicecount != "NA")
wordcounts <- wordcounts[-1,]

# Rename columns with strange names
names(wordcounts)[24]<-"laekkertcount"
names(wordcounts)[27]<-"skoentcount"

# Make all columns numeric
bob <- data.frame(lapply(wordcounts, as.character), stringsAsFactors=FALSE)
wordcountsnum <- data.frame(lapply(bob, as.numeric), stringsAsFactors=FALSE)

#Plot one line for each words
ggplot(wordcountsnum, aes(x = X.step.)) +
  geom_point(aes(y = nicecount), col = "dodgerblue3", alpha = 0.5) +
  geom_point(aes(y = coolcount), col = "red", alpha = 0.5) +
  geom_point(aes(y = funkycount), col = "black", alpha = 0.5) +
  geom_point(aes(y = awesomecount), col = "white", alpha = 0.5) +
  geom_point(aes(y = sweetcount), col = "purple", alpha = 0.5) +
  geom_point(aes(y = fedtcount), col = "pink", alpha = 0.5) +
  geom_point(aes(y = laekkertcount), col = "orange", alpha = 0.5) +
  geom_point(aes(y = dejligtcount), col = "green", alpha = 0.5) +
  geom_point(aes(y = sejtcount), col = "grey", alpha = 0.5) +
  geom_point(aes(y = skoentcount), col = "yellow", alpha = 0.5) +
  ylim(0, 60) +
  labs(y = "Percent",x = "Time")

## With lines - and colored after language? 
ggplot(wordcountsnum, aes(x = X.step.)) +
  geom_line(aes(y = nicecount), col = "dodgerblue3", alpha = 0.5) +
  geom_line(aes(y = coolcount), col = "dodgerblue3", alpha = 0.5) +
  geom_line(aes(y = funkycount), col = "dodgerblue3", alpha = 0.5) +
  geom_line(aes(y = awesomecount), col = "dodgerblue3", alpha = 0.5) +
  geom_line(aes(y = sweetcount), col = "dodgerblue3", alpha = 0.5) +
  geom_line(aes(y = fedtcount), col = "red", alpha = 0.5) +
  geom_line(aes(y = laekkertcount), col = "red", alpha = 0.5) +
  geom_line(aes(y = dejligtcount), col = "red", alpha = 0.5) +
  geom_line(aes(y = sejtcount), col = "red", alpha = 0.5) +
  geom_line(aes(y = skoentcount), col = "red", alpha = 0.5) +
  ylim(0, 60) +
  labs(y = "Percent",x = "Time")

## only the two most common event words and least common words
ggplot(wordcountsnum, aes(x = X.step.)) +
  geom_line(aes(y = nicecount, colour = "nice"), alpha = 0.5) +
  geom_point(aes(y = nicecount, colour = "nice"), size = 0.2) + 
  geom_line(aes(y = coolcount, colour = "cool"), alpha = 0.5) +
  geom_point(aes(y = coolcount, colour = "cool"), size = 0.2) +
  geom_line(aes(y = sejtcount, colour = "sejt"), alpha = 0.5) +
  geom_point(aes(y = sejtcount, colour = "sejt"), size = 0.2) +
  geom_line(aes(y = skoentcount, colour = "skoent"), alpha = 0.5) +
  geom_point(aes(y = skoentcount, colour = "skoent"), size = 0.2) +
  ylim(0, 60) +
  scale_colour_manual("", 
                      breaks = c("nice", "cool", "sejt", "skoent"),
                      values = c("dodgerblue1", "dodgerblue4", "firebrick1", "firebrick4")) +
  labs(title = "Use of most common and least common event words over time", x = "Time", y = "Count (past 250 conversations)") +
  theme(plot.title=element_text(hjust=0.5))
```
...
#### Histogram of event words over time #### (NOT averaged over trials)
#new dataframe equal to eventwords
countevents <- subset(eventwords, X.run.number.== 11)
#define [] as NA
countevents[countevents == "[]"] <- NA
# exclude NAs
counteventword <- subset(countevents, eventword != "NA")
counteventword <- subset(counteventword, eventword != "0")

#Exact number of times each word was the event word
countdf <- counteventword %>% 
  count(eventword)

# subset english and danish event words
eventword_eng <- subset(counteventword, eventword == "nice" | eventword == "cool" | eventword == "awesome" | eventword == "funky" | eventword == "sweet")
eventword_dan <- subset(counteventword, eventword != "nice")
eventword_dan <- subset(eventword_dan, eventword != "cool")
eventword_dan <- subset(eventword_dan, eventword != "awesome")
eventword_dan <- subset(eventword_dan, eventword != "funky")
eventword_dan <- subset(eventword_dan, eventword != "sweet")


#histogram - only eng words
ggplot(eventword_eng, aes(x = eventword)) +
geom_bar(fill="blue",aes(alpha=.7)) +
  labs(title = "Distribution of English event words", x = "word", y = "Count") +
  theme(plot.title=element_text(hjust=0.5))

#histogram - only dan words
ggplot(eventword_dan, aes(x = eventword)) +
geom_bar(fill="red", aes(alpha=0.7)) +
  labs(title = "Distribution of Danish event words", x = "word", y = "Count") + 
  theme(plot.title=element_text(hjust=0.5))

# histogram - all words 
ggplot() +
geom_bar(data = eventword_eng, aes(x = eventword), fill="dodgerblue3",alpha=0.7) +
  geom_bar(data = eventword_dan, aes(x = eventword), fill="red", alpha=0.7) +
  labs(title = "Distribution of event words", x = "Word", y = "Count") +
  theme(plot.title=element_text(hjust=0.5))
```
